...
const tags = { {% for tag in site.tags %}{% capture tag_name %}{{ tag | first }}{% endcapture %}{{ tag_name | replace: " ", "_" }}: [{% for post in site.tags[tag_name] %}{ url: `{{ site.baseurl }}{{ post.url }}`, date: `{{post.date | date_to_string}}`, title: `{{post.title}}`},{% endfor %}],{% endfor %} }

window.onload = function () {
  document.querySelectorAll(".tag").forEach((tag) => {
    tag.addEventListener("click", function (e) {
      const lower_name = e.target.innerText.replace(" ","_");
      ...
      document.querySelector("#tag-modal-title").innerText = tag_name.replace("_"," ");
