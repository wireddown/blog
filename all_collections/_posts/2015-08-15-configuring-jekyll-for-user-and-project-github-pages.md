...
> Note (May 2026): This post has been edited to improve how it renders in the new theme.
...
For a User site, `baseurl` must be empty
...
For a Project site, `baseurl` must begin with a slash
...
Always concatenate Jekyll and Liquid tags
...
*(Almost)* Always start links with `{% raw %}{{ site.baseurl }}{% endraw %}`.
...
Start hyperlinks with `{% raw %}{{ site.url }}{{ site.baseurl }}{% endraw %}` in feed pages, like atom.xml.

Otherwise, feed readers and other aggregators, which rely on absolute URLs, won't be able to send subscribers to your pages.

### Rule 3

Always use a trailing slash after `{% raw %}{{ site.baseurl }}{% endraw %}`
